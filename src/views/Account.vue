    ...
    #Account.main-page.detail-page
        ...
        div.height_100(v-show='!loading')
            ...
            v-row#page-head.d-flex.align-center.my-0
                v-col(cols='12' lg='8')
                    .d-flex.align-center
                        v-icon.mr-2(size='40' color='black') mdi-domain
                        h1.my-0 {{account.name}}
                v-col(cols='12' lg='4')
                    v-row.justify-md-end(no-gutters='no-gutters')
                        v-col.ml-2(cols='auto')
                            .btn-group
                                v-btn(@click='showEditAccount(account)' width='60' height='30' color='white' depressed tile dark)
                                    span 編輯
            ...
            #page-inner.pa-3.pt-0
                v-card.card-preface.card-shadow.px-0.py-6(flat)
                    v-row
                        v-col.px-2.px-md-4.py-0(cols='6' md='3')
                            v-list-item.d-flex.align-center.px-0
                                v-list-item-icon.mt-3.mb-0.mr-2
                                    v-icon(size='28') mdi-phone
                                v-list-item-content.pa-0
                                    v-card-subtitle.pa-0.t-lightgary 電話
                                    a.f-text-highlight.mb-0(:href='onCall(account.phone)' target='_blank') {{ account.phone }}
                        v-col.px-2.px-md-4.py-0.border-left(cols='6' md='3')
                            v-list-item.d-flex.align-center.px-0
                                v-list-item-icon.mt-3.mb-0.mr-2
                                    v-icon(size='28') mdi-map
                                v-list-item-content.pa-0
                                    v-card-subtitle.pa-0.t-lightgary 地址
                                    a.f-text-highlight.mb-0(:href='account.addressLink' target='_blank') {{ account.address }}
                        v-col.px-2.px-md-4.py-0.border-left(cols='6' md='3')
                            v-list-item.d-flex.align-center.px-0
                                v-list-item-icon.mt-3.mb-0.mr-2
                                    v-icon(size='28') mdi-earth
                                v-list-item-content.pa-0
                                    v-card-subtitle.pa-0.t-lightgary 網站
                                    a.f-text-highlight.mb-0(:href='account.web' target='_blank') {{ account.web }}
                        v-col.px-2.px-md-4.py-0.border-left(cols='12' md='3')
                            v-list-item.d-flex.align-center.pa-0
                                v-list-item-avatar.ma-0.mr-2(color='aliceBlue')
                                    span.mx-auto(v-if="account.ownerPic === ''") {{ account.ownerNickName }}
                                    img(v-else :src='account.ownerPic' :alt='account.ownerName')
                                v-list-item-content.pa-0
                                    v-card-subtitle.pa-0.t-lightgary 機會擁有者
                                    a.t-primary.mb-0.f-text-highlight(href='/Account') {{ account.ownerName }}
                        ...
                        v-card.deal-tab-card(flat)
                            v-tabs.main-tabs(v-model='tab' background-color='transparent' :prev-icon="prevIcon ? 'mdi-arrow-left-bold-box-outline' : ''" :next-icon="nextIcon ? 'mdi-arrow-right-bold-box-outline' : ''" show-arrows height='53')
                                ...
                                v-tab-item.pa-3(key='detail' value='tab-1' :transition='false' :reverse-transition='false')
                                    .d-flex.align-center.justify-space-between.mt-2.mb-4
                                        h3.d-flex.align-center
                                            ...
                                            span 商機
                                        v-btn.pa-0(v-if='account.deals.length >= 4' @click='moreDeals' color='primary' text) MORE
                                    v-row.small-deals-box(:class="showDealsMore ? 'show-more':''")
                                        v-col.px-1.py-3(cols='12' md='4' v-for='(dealsItem, index) in account.deals' :key='index')
                                        ...
                                        v-col.pr-md-4(cols='12' md='6')
                                            div.d-flex.align-center.justify-space-between.input-has-label
                                                .d-inline-flex.align-center
                                                    ...
                                                    v-text-field#client(v-if='!isDetailReadonly' v-model='account.name' solo='solo' readonly)
                                                    a.t-primary(v-else href='/Account') {{ account.name }}
                                            ...
                                            p.d-flex.align-center.input-has-label
                                                label(for='type') 類型
                                                v-text-field#type(v-model='account.type' solo='solo' :readonly='isDetailReadonly')
                                                v-btn.ml-1(v-if='showDetailEditBtn' icon color='gary' @click='onEditDetail')
                                                    v-icon(size='18') icon-edit
                                            div.d-flex.align-center.justify-space-between.input-has-label
                                                .d-inline-flex.align-center
                                                    label(for='web') 網站
                                                    v-text-field#web(v-if='!isDetailReadonly' v-model='account.web' solo='solo')
                                                    a.t-primary(v-else href='account.web' target='_blank') {{ account.web }}
                                                v-btn.ml-1(v-if='showDetailEditBtn' icon color='gary' @click='onEditDetail')
                                                    v-icon(size='18') icon-edit
                                            p.d-flex.align-center.input-has-label
                                                label(for='describe') 描述
                                                v-text-field#describe(v-model='account.describe' solo='solo' :readonly='isDetailReadonly')
                                                v-btn.ml-1(v-if='showDetailEditBtn' icon color='gary' @click='onEditDetail')
                                                    v-icon(size='18') icon-edit
                                        v-col.pl-md-4(cols='12' md='6')
                                            p.d-flex.align-center.input-has-label
                                                label(for='own') 帳戶擁有者
                                                v-text-field#own(v-model='account.ownerName' solo='solo' :readonly='isDetailReadonly')
                                                v-btn.ml-1(v-if='showDetailEditBtn' icon color='gary' @click='onEditDetail')
                                                    v-icon(size='18') icon-edit
                                            div.d-flex.align-center.justify-space-between.input-has-label
                                                .d-inline-flex.align-center
                                                    label(for='cell') 電話
                                                    v-text-field#cell(v-if='!isDetailReadonly' v-model='account.phone' solo='solo')
                                                    a.t-primary(v-else :href='onCall(account.phone)') {{ account.phone }}
                                                v-btn.ml-1(v-if='showDetailEditBtn' icon color='gary' @click='onEditDetail')
                                                    v-icon(size='18') icon-edit
                                            p.d-flex.align-center.input-has-label
                                                label(for='industry') 產業
                                                v-text-field#industry(v-model='account.industry' solo='solo' :readonly='isDetailReadonly')
                                                v-btn.ml-1(v-if='showDetailEditBtn' icon color='gary' @click='onEditDetail')
                                                    v-icon(size='18') icon-edit
                                            p.d-flex.align-center.input-has-label
                                                label(for='peoples') 員工
                                                v-text-field#peoples(v-model='account.peoples' solo='solo' :readonly='isDetailReadonly')
                                                v-btn.ml-1(v-if='showDetailEditBtn' icon color='gary' @click='onEditDetail')
                                                    v-icon(size='18') icon-edit
                                ...
                                v-tab-item.pa-3(key='timeline' value='tab-2' :transition='false' :reverse-transition='false')
                                    v-tabs.borderBtn-tabs(height='32' background-color='white' hide-slider show-arrows centered)
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            onDelete () {
                this.showMessageDialog('您確定要刪除此帳戶嗎？');
            },
            onCopy() {
                this.showMessageDialog('您確定要複製此帳戶嗎？');
            },
            showEditAccount() {
